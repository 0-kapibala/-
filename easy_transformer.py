...
from DL.transformer_translation.models import MultiHeadAttention


class MultiHeadAttention(nn.Moudle):
    def __init__(self,d_model,num_heads):
        super(MultiHeadAttention,self).__init__()
        self.num_heads = num_heads#模型维度
        self.d_model = d_model#注意力头数
        self.d_k = d_model // num_heads#每个维度数
    ...
    def forward(self,Q,K,V,mask=None):
        ...
        atten_out=self.scaled_dot_product_attention(Q,K,V,mask)
        O=self.W_o(self.combine_heads(atten_out))
        return O
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_seq_length):
        ...
        postion=torch.arange(0,max_seq_length,dtype=torch.float32).unsqueeze(1)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self,x,mask):
        self.atten_output=self.self_atten(x,x,x,mask)
        x=self.norm1(x+self.dropout(self.atten_output))

        self.feed_forward_output=self.feed_forward(x)
        x=self.norm2(x+self.dropout(self.feed_forward_output))
        return x
